...
static void BufFileCheckPointer(BufFile* file)
{
    MemoryContext memory_context = GetMemoryChunkContext(file);

    /* AMM chunks have a standard context header but no AllocSet debug trailer. */
    if (IsA(memory_context, AmmGranuleContext))
        AmmGranuleContextCheckPointer(memory_context, file);
    else
