...
typedef struct BufferStrategyControl {
    ...
    pg_atomic_uint32 activeBufferCount; /* Runtime normal shared-buffer envelope */
...
int StrategyActiveBufferCount(void)
{
    uint32 active_buffers = pg_atomic_read_u32(&t_thrd.storage_cxt.StrategyControl->activeBufferCount);

    if (active_buffers == 0 || active_buffers > (uint32)NORMAL_SHARED_BUFFER_NUM)
        return NORMAL_SHARED_BUFFER_NUM;

    return (int)active_buffers;
}

void StrategySetActiveBufferCount(int active_buffers)
{
    active_buffers = Max(active_buffers, 1);
    active_buffers = Min(active_buffers, NORMAL_SHARED_BUFFER_NUM);
    pg_atomic_write_u32(&t_thrd.storage_cxt.StrategyControl->activeBufferCount, (uint32)active_buffers);
}

static inline bool StrategyBufferIdIsActive(int buf_id)
{
    return GsAmmBufferIdIsActive(buf_id);
}

static inline bool StrategyBufferNumberIsActive(Buffer buf_num)
{
    return GsAmmBufferNumberIsActive(buf_num);
}
...
retry:
    ...
    max_buffer_can_use = StrategyActiveBufferCount();
    if (am_standby)
        max_buffer_can_use = Min(max_buffer_can_use,
            int(NORMAL_SHARED_BUFFER_NUM * u_sess->attr.attr_storage.shared_buffers_fraction));
    max_buffer_can_use = Max(max_buffer_can_use, 1);
    ...
    for (;;) {
        ...
        if (!GsAmmBufferIdIsActive(buf->buf_id)) {
            if (--try_counter == 0) {
                try_counter = max_buffer_can_use;
...
void StrategyInitialize(bool init)
{
    ...
    if (!found) {
        ...
        pg_atomic_init_u32(&t_thrd.storage_cxt.StrategyControl->activeBufferCount, NORMAL_SHARED_BUFFER_NUM);
...
RETRY:
    ...
    if (!StrategyBufferNumberIsActive(buf_num)) {
...
static BufferDesc* get_buf_from_candidate_list(BufferAccessStrategy strategy, uint64* buf_state)
{
    ...
    for (int i = 0; i < list_num; i++) {
        ...
        while (candidate_buf_pop(&g_instance.ckpt_cxt_ctl->pgwr_procs.writer_proc[thread_id].normal_list, &buf_id)) {
            ...
            if (!StrategyBufferIdIsActive(buf_id)) {
