...
int BufTableLookup(BufferTag *tag, uint32 hashcode)
{
    ...
    if (!GsAmmBufferIdGetActiveToken(result->id, &token)) {
        return -1;
    }
    if (GsAmmBufferIdIsInBufferGranule(result->id) &&
        (result->amm_owner_epoch != token.owner_epoch ||
            GetBufferDescriptor(result->id)->extra->amm_owner_epoch != token.owner_epoch)) {
        return -1;
    }
    if (!GsAmmBufferTokenIsActive(result->id, token)) {
        return -1;
    }
...
int BufTableInsert(BufferTag *tag, uint32 hashcode, int buf_id)
{
    ...
    result->id = buf_id;
    result->amm_owner_epoch = token.owner_epoch;
    GetBufferDescriptor(buf_id)->extra->amm_owner_epoch = token.owner_epoch;
