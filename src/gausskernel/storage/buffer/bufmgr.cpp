...
static bool ReadBuffer_common_ReadBlock(SMgrRelation smgr, char relpersistence, ForkNumber forkNum,
    ...
    bool count_shared_physical_read, bool *need_repair);
...
static inline bool GsAmmBufferIdCanMaintain(int buf_id)
{
    return GsAmmBufferIdIsActive(buf_id) || GsAmmBufferIdIsDraining(buf_id);
}
                                                ...
                                                BlockNumber block_num, BufferAccessStrategy strategy, bool *found)
{
    ...
    for (;;) {
        ...
        if (buf_id >= 0) {
            if (!GsAmmBufferIdIsActive(buf_id)) {
                UnpinBuffer(buf, true);
    ...
    do {
        ...
        for (int i = 0; i < nbufs; i++) {
            ...
            if (strategy_writes) {
                ...
                buf_id = (int)buf_list[i] - 1;
                if (!GsAmmBufferIdCanMaintain(buf_id)) {
                    if (reorder_writes) {
                                      ...
                                      ReadBufferMode mode, BufferAccessStrategy strategy, bool *hit)
{
    ...
    *hit = false;
    ...
    (void)ReadBuffer_common_ReadBlock(smgr, relpersistence, forkNum, blockNum, mode,
                                      isExtend, bufBlock, NULL, false, &need_reapir);
    ...
    const XLogPhyBlock *pblk, Block bufBlock)
{
    ...
    if (pblk != NULL) {
        ...
        (void)ReadBuffer_common_ReadBlock(smgr, relpersistence, forkNum,
            blockNum, readmode, isExtend, bufBlock, pblk, true, &need_repair);
    } else {
        (void)ReadBuffer_common_ReadBlock(smgr, relpersistence, forkNum,
            blockNum, readmode, isExtend, bufBlock, NULL, true, &need_repair);
    }
    if (need_repair) {
...
found_branch:
    ...
    bool needputtodirty = ReadBuffer_common_ReadBlock(smgr, relpersistence, forkNum, blockNum,
                                                      mode, isExtend, bufBlock, pblk, !isLocalBuf, &need_repair);
    if (need_repair) {
...
static bool PinBufferForAmmLookup(BufferDesc *buf, BufferAccessStrategy strategy, bool *valid)
{
    GsAmmGranuleToken token;
    int buf_id;

    if (buf == NULL || valid == NULL)
        return false;
    buf_id = buf->buf_id;
    if (!GsAmmBufferIdGetActiveToken(buf_id, &token))
        return false;

    *valid = PinBuffer(buf, strategy);
    if (GsAmmBufferTokenIsActive(buf_id, token))
        return true;
...
retry:
    ...
    if (buf_id >= 0) {
        ...
        if (!PinBufferForAmmLookup(buf, strategy, &valid))
            goto retry;
...
{    
    ...
    if (!GsAmmBufferIdCanMaintain(buf->buf_id)) {
        UnlockBufHdr(buf, buf_state);
        return;
    }
...
static FORCE_INLINE void ScanCompareAndInvalidateBuffer(const RelFileNode *rnodes, int rnode_len, int buffer_idx)
{
    ...
    if (!GsAmmBufferIdCanMaintain(buffer_idx))
        return;
...
{
    uint64 buf_state;
    if (!GsAmmBufferIdCanMaintain(buf_desc->buf_id))
        return;
