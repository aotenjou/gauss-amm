...
static bool ExecHashSkewTableInsert(HashJoinTable hashtable, TupleTableSlot* slot, uint32 hashvalue, int bucketNumber);
static void ExecHashRemoveNextSkewBucket(HashJoinTable hashtable);
static void ExecHashIncreaseBuckets(HashJoinTable hashtable);

#define HASH_CHUNK_DEDICATED_TUPLES (-1)

static bool ExecHashEnsureTupleCapacity(
    HashJoinTable hashtable, Size tupleBytes, uint32 hashvalue, int* bucketno, int* batchno);
static void* ExecHashAmmTryAlloc(MemoryContext context, Size size);
static void* ExecHashAmmTryAllocZero(MemoryContext context, Size size);
static bool ExecHashAmmEnsureBatchFileSegment(HashJoinTable hashtable, int segmentNo);
static bool ExecHashAmmTryPrepareBatchFiles(HashJoinTable hashtable, int nbatch);
static bool ExecHashAmmTryResizeBuckets(HashJoinTable hashtable, int nbuckets);
static Size ExecHashAmmChunkPayloadSize(HashJoinTable hashtable, Size tupleBytes);
static bool ExecHashChunkIsDedicated(HashMemoryChunk chunk);
static void ExecHashPackAmmNormalChunks(
    HashJoinTable hashtable, HashMemoryChunk normalChunks, HashMemoryChunk dedicatedChunks);
static void ExecHashRebatchAmmChunks(
    HashJoinTable hashtable, HashMemoryChunk oldchunks, int curbatch, long* ninmemory, long* nfreed);
static void* dense_alloc_try(HashJoinTable hashtable, Size size);

static int gs_amm_hash_multipass_count(HashJoinTable hashtable)
{
    int initial_batches = Max(hashtable->nbatch_original, 1);
    int final_batches = Max(hashtable->nbatch, 1);
    int multipass_count = initial_batches > 1 ? 1 : 0;

    while (initial_batches < final_batches) {
        multipass_count++;
        if (initial_batches > INT_MAX / 2)
            break;
        initial_batches *= 2;
    }
    return multipass_count;
}

static void GsAmmApplyHashEffectiveGrant(HashJoinTable hashtable)
{
...
static void ExecHashTupleGrantError(HashJoinTable hashtable, Size tupleBytes, int sqlState)
{
    ereport(ERROR,
        (errcode(sqlState),
            ...
            errmsg("hash join tuple exceeds AMM memory grant: tuple bytes %lu, grant bytes %lu, nbatch %d",
                (unsigned long)tupleBytes,
                (unsigned long)hashtable->spaceAllowed,
                hashtable->nbatch)));
}

static void ExecHashUnbatchableGrantError(HashJoinTable hashtable, Size bucketBytes)
{
    ereport(ERROR,
        (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
            errmodule(MOD_EXECUTOR),
            errmsg("hash join cannot load a batch within AMM memory grant after batching: bucket bytes %lu, grant bytes %lu, nbatch %d",
                (unsigned long)bucketBytes,
                (unsigned long)hashtable->spaceAllowed,
                hashtable->nbatch)));
}

static void ExecHashMinimumBucketGrantError(HashJoinTable hashtable, Size bucketBytes)
{
    ereport(ERROR,
        (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
            errmodule(MOD_EXECUTOR),
            errmsg("hash join cannot allocate its minimum bucket array within AMM memory grant: bucket bytes %lu, grant bytes %lu",
                (unsigned long)bucketBytes,
                (unsigned long)hashtable->spaceAllowed)));
}

static void ExecHashBatchMetadataGrantError(HashJoinTable hashtable)
{
    ereport(ERROR,
        (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
            errmodule(MOD_EXECUTOR),
            errmsg("hash join batch metadata cannot expand within AMM memory grant; no further batching is possible: grant bytes %lu, nbatch %d",
                (unsigned long)hashtable->spaceAllowed,
                hashtable->nbatch)));
}
...
static void* ExecHashAmmTryAlloc(MemoryContext context, Size size)
{
    if (!AmmGranuleContextCanAllocate(context, size))
        return NULL;
    return AmmGranuleContextTryAlloc(context, size);
}
...
static void* ExecHashAmmTryAllocZero(MemoryContext context, Size size)
{
    ...
    if (!AmmGranuleContextCanAllocate(context, size))
        return NULL;
    pointer = AmmGranuleContextTryAlloc(context, size);
    if (pointer == NULL)
        return NULL;
    rc = memset_s(pointer, size, 0, size);
...
static bool ExecHashAmmEnsureBatchFileSegment(HashJoinTable hashtable, int segmentNo)
{
    HashJoinBatchFileSegment* segment;

    if (segmentNo < 0 || segmentNo >= HASHJOIN_AMM_BATCH_FILE_SEGMENT_COUNT)
        return false;
    if (hashtable->ammBatchFileSegments[segmentNo] != NULL)
        return true;

    segment = (HashJoinBatchFileSegment*)ExecHashAmmTryAllocZero(
        hashtable->hashCxt, sizeof(HashJoinBatchFileSegment));
    if (segment == NULL)
        return false;
    hashtable->ammBatchFileSegments[segmentNo] = segment;
    return true;
}

bool ExecHashUsesAmmBatchFileSegments(HashJoinTable hashtable)
{
    return hashtable != NULL && hashtable->ammBatchFileSegmentsEnabled;
}

static HashJoinBatchFileSegment* ExecHashGetAmmBatchFileSegment(HashJoinTable hashtable, int batchno)
{
    int segmentNo;

    Assert(ExecHashUsesAmmBatchFileSegments(hashtable));
    Assert(batchno >= 0);
    segmentNo = batchno / HASHJOIN_AMM_BATCH_FILE_SEGMENT_SLOTS;
    if (segmentNo >= HASHJOIN_AMM_BATCH_FILE_SEGMENT_COUNT ||
        hashtable->ammBatchFileSegments[segmentNo] == NULL)
        ereport(ERROR,
            (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
                errmodule(MOD_EXECUTOR),
                errmsg("hash join batch metadata cannot continue batching within AMM memory grant")));
    return hashtable->ammBatchFileSegments[segmentNo];
}

BufFile** ExecHashGetInnerBatchFileSlot(HashJoinTable hashtable, int batchno)
{
    HashJoinBatchFileSegment* segment;

    if (!ExecHashUsesAmmBatchFileSegments(hashtable))
        return &hashtable->innerBatchFile[batchno];
    segment = ExecHashGetAmmBatchFileSegment(hashtable, batchno);
    return &segment->inner[batchno % HASHJOIN_AMM_BATCH_FILE_SEGMENT_SLOTS];
}

BufFile** ExecHashGetOuterBatchFileSlot(HashJoinTable hashtable, int batchno)
{
    HashJoinBatchFileSegment* segment;

    if (!ExecHashUsesAmmBatchFileSegments(hashtable))
        return &hashtable->outerBatchFile[batchno];
    segment = ExecHashGetAmmBatchFileSegment(hashtable, batchno);
    return &segment->outer[batchno % HASHJOIN_AMM_BATCH_FILE_SEGMENT_SLOTS];
}
...
static bool ExecHashAmmTryPrepareBatchFiles(HashJoinTable hashtable, int nbatch)
{
    ...
    if (nbatch <= 1)
        return true;
    lastSegment = (nbatch - 1) / HASHJOIN_AMM_BATCH_FILE_SEGMENT_SLOTS;
    for (int segmentNo = 0; segmentNo <= lastSegment; segmentNo++) {
        if (!ExecHashAmmEnsureBatchFileSegment(hashtable, segmentNo))
            return false;
    }

    hashtable->ammBatchFileSegmentsEnabled = true;
    return true;
}

static bool ExecHashAmmTryResizeBuckets(HashJoinTable hashtable, int nbuckets)
{
    Size bytes = (Size)nbuckets * sizeof(HashJoinTuple);
    HashJoinTuple* buckets = (HashJoinTuple*)ExecHashAmmTryAllocZero(hashtable->batchCxt, bytes);

    if (buckets == NULL)
        return false;
    pfree_ext(hashtable->buckets);
    hashtable->buckets = buckets;
    hashtable->nbuckets = nbuckets;
    hashtable->log2_nbuckets = my_log2(nbuckets);
    return true;
}

static Size ExecHashAmmChunkPayloadSize(HashJoinTable hashtable, Size tupleBytes)
{
    Size minimum = MAXALIGN(tupleBytes);
    Size candidate = HASH_CHUNK_SIZE;

    if (candidate < minimum)
        candidate = minimum;
    while (candidate > minimum) {
        if (AmmGranuleContextCanAllocate(
                hashtable->batchCxt, offsetof(HashMemoryChunkData, data) + candidate))
            return candidate;
        candidate >>= 1;
        if (candidate < minimum)
            candidate = minimum;
    }
    if (AmmGranuleContextCanAllocate(hashtable->batchCxt, offsetof(HashMemoryChunkData, data) + minimum))
        return minimum;
    return 0;
}
...
static bool ExecHashChunkIsDedicated(HashMemoryChunk chunk)
{
    return chunk->ntuples == HASH_CHUNK_DEDICATED_TUPLES;
}

static Size dense_alloc_new_chunk_size(HashJoinTable hashtable, Size size)
{
    size = MAXALIGN(size);

    if (size > HASH_CHUNK_THRESHOLD)
        return offsetof(HashMemoryChunkData, data) + size;
    ...
    HashJoinTable hashtable, Size tupleBytes, uint32 hashvalue, int* bucketno, int* batchno)
{
    ...
    GsAmmApplyHashEffectiveGrant(hashtable);
    if (tupleBytes > MaxAllocSize - offsetof(HashMemoryChunkData, data))
        ExecHashTupleGrantError(hashtable, tupleBytes, ERRCODE_PROGRAM_LIMIT_EXCEEDED);

    while (*batchno == hashtable->curbatch) {
        minimumChunkBytes = dense_alloc_new_chunk_size(hashtable, tupleBytes);
        bucketBytes = (int64)hashtable->nbuckets_optimal * sizeof(HashJoinTuple);
        spaceAvailable = tupleBytes <= (Size)hashtable->spaceAllowed &&
            hashtable->spaceUsed <= hashtable->spaceAllowed - (int64)tupleBytes &&
            bucketBytes <= hashtable->spaceAllowed - hashtable->spaceUsed - (int64)tupleBytes;
        if (minimumChunkBytes == 0) {
            chunkAvailable = true;
        } else if (tupleBytes > HASH_CHUNK_THRESHOLD) {
            chunkAvailable = AmmGranuleContextCanAllocate(hashtable->batchCxt, minimumChunkBytes);
        } else {
            chunkAvailable = ExecHashAmmChunkPayloadSize(hashtable, tupleBytes) != 0;
        }

        if (spaceAvailable && chunkAvailable)
            return true;

        if (minimumChunkBytes > (Size)hashtable->spaceAllowed)
            ExecHashTupleGrantError(hashtable, tupleBytes, ERRCODE_PROGRAM_LIMIT_EXCEEDED);

        /* Dense skew tuples must be detached before a chunk scan can rebatch them. */
        if (hashtable->skewEnabled) {
            while (hashtable->skewEnabled)
    ...
    for (;;) {
        ...
        if (ExecHashGetHashValue(hashtable, econtext, hashkeys, false, hashtable->keepNulls, &hashvalue)) {
            ...
            if (bucketNumber != INVALID_SKEW_BUCKET_NO) {
                ...
                if (ExecHashSkewTableInsert(hashtable, slot, hashvalue, bucketNumber))
...
HashJoinTable ExecHashTableCreate(Hash* node, List* hashOperators, bool keepNulls, List *hash_collations)
{
    ...
    int amm_grant_kb = GsAmmCurrentBackendGrantKB();
    ...
    hashtable->ammBatchFileSegmentsEnabled = false;
    for (i = 0; i < HASHJOIN_AMM_BATCH_FILE_SEGMENT_COUNT; i++)
        hashtable->ammBatchFileSegments[i] = NULL;
    ...
    if (amm_grant_id > 0) {
        uint64 context_bytes = Max(GsAmmCurrentBackendGrantPoolBytes(),
            Max((uint64)local_work_mem, (uint64)64) * 1024);

        hashtable->hashCxt = AmmGranuleContextCreate(CurrentMemoryContext, "HashTableContext", amm_grant_id,
            context_bytes);
        hashtable->batchCxt = AmmGranuleContextCreate(hashtable->hashCxt, "HashBatchContext", amm_grant_id,
            context_bytes);
    } else {
    ...
    if (nbatch > 1) {
         ...
         */
        if (IsA(hashtable->hashCxt, AmmGranuleContext)) {
            if (!ExecHashAmmTryPrepareBatchFiles(hashtable, nbatch)) {
                nbatch = 1;
                hashtable->nbatch = 1;
    ...
    if (IsA(hashtable->batchCxt, AmmGranuleContext)) {
        while (hashtable->buckets == NULL) {
            Size bucketBytes = (Size)nbuckets * sizeof(HashJoinTuple);

            hashtable->buckets = (HashJoinTuple*)ExecHashAmmTryAllocZero(hashtable->batchCxt, bucketBytes);
            if (hashtable->buckets != NULL)
                break;
            if (nbuckets == 1)
                ExecHashMinimumBucketGrantError(hashtable, bucketBytes);
            nbuckets >>= 1;
        }
        hashtable->nbuckets = nbuckets;
        hashtable->nbuckets_original = nbuckets;
...
void ExecHashTableDestroy(HashJoinTable hashtable)
{
    ...
    for (i = 1; i < hashtable->nbatch; i++) {
        BufFile** innerSlot = ExecHashGetInnerBatchFileSlot(hashtable, i);
        BufFile** outerSlot = ExecHashGetOuterBatchFileSlot(hashtable, i);

        if (*innerSlot)
            BufFileClose(*innerSlot);
        if (*outerSlot)
            BufFileClose(*outerSlot);
    }
...
/*
 * Pack the current-batch tuples from normal AMM chunks into the earliest
 * processed chunk with room.  A source chunk only becomes a destination after
 * its prior bytes have been read, so in-chunk moves always go toward lower
 * offsets and no future source data is overwritten.
 */
static void ExecHashPackAmmNormalChunks(
    HashJoinTable hashtable, HashMemoryChunk normalChunks, HashMemoryChunk dedicatedChunks)
{
    HashMemoryChunk sourceChunk = normalChunks;
    HashMemoryChunk writableNormalChunk = NULL;
    HashMemoryChunk fullNormalChunks = NULL;
    HashMemoryChunk fullNormalChunksTail = NULL;

    while (sourceChunk != NULL) {
        HashMemoryChunk sourceNext = sourceChunk->next;
        Size sourceUsed = sourceChunk->used;
        Size sourceReadOffset = 0;
        bool sourceRetained = false;

        while (sourceReadOffset < sourceUsed) {
            HashJoinTuple sourceTuple = (HashJoinTuple)(sourceChunk->data + sourceReadOffset);
            MinimalTuple tuple = HJTUPLE_MINTUPLE(sourceTuple);
            Size tupleSize = HJTUPLE_OVERHEAD + tuple->t_len;
            Size alignedTupleSize = MAXALIGN(tupleSize);
            Size destinationOffset;
            HashJoinTuple destinationTuple;
            int bucketno;
            int batchno;
            errno_t rc;

            if (writableNormalChunk == NULL ||
                writableNormalChunk->maxlen - writableNormalChunk->used < alignedTupleSize) {
                if (writableNormalChunk != NULL) {
                    Assert(writableNormalChunk->used == writableNormalChunk->maxlen);
                    writableNormalChunk->next = NULL;
                    if (fullNormalChunks == NULL)
                        fullNormalChunks = writableNormalChunk;
                    else
                        fullNormalChunksTail->next = writableNormalChunk;
                    fullNormalChunksTail = writableNormalChunk;
                }

                writableNormalChunk = sourceChunk;
                writableNormalChunk->used = 0;
                writableNormalChunk->ntuples = 0;
                sourceRetained = true;
            }

            destinationOffset = writableNormalChunk->used;
            Assert(writableNormalChunk != sourceChunk || destinationOffset <= sourceReadOffset);
            destinationTuple = (HashJoinTuple)(writableNormalChunk->data + destinationOffset);
            if (destinationTuple != sourceTuple) {
                rc = memmove_s(destinationTuple,
                    writableNormalChunk->maxlen - destinationOffset,
                    sourceTuple,
                    tupleSize);
                securec_check(rc, "\0", "\0");
            }

            ExecHashGetBucketAndBatch(hashtable, destinationTuple->hashvalue, &bucketno, &batchno);
            Assert(batchno == hashtable->curbatch);
            destinationTuple->next = hashtable->buckets[bucketno];
            hashtable->buckets[bucketno] = destinationTuple;
            writableNormalChunk->used += alignedTupleSize;
            writableNormalChunk->ntuples++;
            sourceReadOffset += alignedTupleSize;
            CHECK_FOR_INTERRUPTS();
        }

        if (writableNormalChunk != NULL && writableNormalChunk->used == writableNormalChunk->maxlen) {
            writableNormalChunk->next = NULL;
            if (fullNormalChunks == NULL)
                fullNormalChunks = writableNormalChunk;
            else
                fullNormalChunksTail->next = writableNormalChunk;
            fullNormalChunksTail = writableNormalChunk;
            if (writableNormalChunk == sourceChunk)
                sourceRetained = true;
            writableNormalChunk = NULL;
        }

        if (!sourceRetained)
            pfree_ext(sourceChunk);
        sourceChunk = sourceNext;
    }

    if (writableNormalChunk != NULL) {
        writableNormalChunk->next = fullNormalChunks;
        if (fullNormalChunksTail != NULL)
            fullNormalChunksTail->next = dedicatedChunks;
        else
            writableNormalChunk->next = dedicatedChunks;
        hashtable->chunks = writableNormalChunk;
    } else if (fullNormalChunks != NULL) {
        fullNormalChunksTail->next = dedicatedChunks;
        hashtable->chunks = fullNormalChunks;
    } else {
        hashtable->chunks = dedicatedChunks;
    }
}

/*
 * Rebatch AMM tuples without allocating replacement chunks.  First compact
 * each source locally while spilling future batches, then pack normal chunks
 * globally so the remaining head can serve the next dense allocation.
 */
static void ExecHashRebatchAmmChunks(
    HashJoinTable hashtable, HashMemoryChunk oldchunks, int curbatch, long* ninmemory, long* nfreed)
{
    HashMemoryChunk dedicatedChunks = NULL;

    while (oldchunks != NULL) {
        HashMemoryChunk oldchunk = oldchunks;
        HashMemoryChunk nextchunk = oldchunk->next;
        size_t readOffset = 0;
        size_t writeOffset = 0;
        int keptTuples = 0;

        if (ExecHashChunkIsDedicated(oldchunk)) {
            HashJoinTuple hashTuple = (HashJoinTuple)oldchunk->data;
            MinimalTuple tuple = HJTUPLE_MINTUPLE(hashTuple);
            Size hashTupleSize = HJTUPLE_OVERHEAD + tuple->t_len;
            int bucketno;
            int batchno;

            if (hashTuple->next != hashTuple) {
                (*ninmemory)++;
                ExecHashGetBucketAndBatch(hashtable, hashTuple->hashvalue, &bucketno, &batchno);
                if (batchno == curbatch) {
                    hashTuple->next = hashtable->buckets[bucketno];
                    hashtable->buckets[bucketno] = hashTuple;
                    oldchunk->next = dedicatedChunks;
                    dedicatedChunks = oldchunk;
                } else {
                    Assert(batchno > curbatch);
                    ExecHashJoinSaveTuple(
                        tuple, hashTuple->hashvalue, ExecHashGetInnerBatchFileSlot(hashtable, batchno));
        ...
        while (readOffset < oldchunk->used) {
            HashJoinTuple hashTuple = (HashJoinTuple)(oldchunk->data + readOffset);
            MinimalTuple tuple = HJTUPLE_MINTUPLE(hashTuple);
            Size hashTupleSize = HJTUPLE_OVERHEAD + tuple->t_len;
            Size alignedTupleSize = MAXALIGN(hashTupleSize);
            int bucketno;
            int batchno;

            if (hashTuple->next == hashTuple) {
                readOffset += alignedTupleSize;
                continue;
            }
            ...
            if (batchno == curbatch) {
                HashJoinTuple keptTuple = (HashJoinTuple)(oldchunk->data + writeOffset);
                errno_t rc;

                Assert(writeOffset <= readOffset);
                if (keptTuple != hashTuple) {
                    rc = memmove_s(keptTuple, oldchunk->maxlen - writeOffset, hashTuple, hashTupleSize);
                    securec_check(rc, "\0", "\0");
                }
                writeOffset += alignedTupleSize;
                keptTuples++;
            } else {
                Assert(batchno > curbatch);
                ExecHashJoinSaveTuple(
        ...
        oldchunk->used = writeOffset;
        oldchunk->ntuples = keptTuples;
        if (keptTuples == 0) {
            pfree_ext(oldchunk);
        } else {
            oldchunk->next = hashtable->chunks;
            hashtable->chunks = oldchunk;
        }
        oldchunks = nextchunk;
    }

    {
        HashMemoryChunk normalChunks = hashtable->chunks;

        hashtable->chunks = NULL;
        ExecHashPackAmmNormalChunks(hashtable, normalChunks, dedicatedChunks);
    }
}
...
static void ExecHashIncreaseNumBatches(HashJoinTable hashtable)
{
    ...
    if (IsA(hashtable->hashCxt, AmmGranuleContext)) {
        if (!ExecHashAmmTryPrepareBatchFiles(hashtable, nbatch)) {
            ...
            ExecHashBatchMetadataGrantError(hashtable);
        }
        PrepareTempTablespaces();
    } else {
    ...
    if (hashtable->nbuckets_optimal != hashtable->nbuckets) {
        ...
        if (IsA(hashtable->batchCxt, AmmGranuleContext)) {
            if (!ExecHashAmmTryResizeBuckets(hashtable, hashtable->nbuckets_optimal)) {
    ...
    if (IsA(hashtable->batchCxt, AmmGranuleContext)) {
        ...
        while (oldchunks != NULL) {
            ...
            while (idx < oldchunks->used) {
                ...
                if (batchno == curbatch) {
                    HashJoinTuple copyTuple = (HashJoinTuple)dense_alloc_try(hashtable, hashTupleSize);
                    ...
                    ExecHashJoinSaveTuple(HJTUPLE_MINTUPLE(hashTuple),
                        hashTuple->hashvalue,
                        ExecHashGetInnerBatchFileSlot(hashtable, batchno));
                    hashtable->spaceUsed -= hashTupleSize;
                    nfreed++;
                }
                         ...
                         Instrumentation *instrument)
{
    ...
    errno_t errorno = EOK;
    Size hashTupleSize = HJTUPLE_OVERHEAD + tuple->t_len;
    bool ammBatchContext = IsA(hashtable->batchCxt, AmmGranuleContext);
    HashJoinTuple hashTuple = NULL;

    ExecHashGetBucketAndBatch(hashtable, hashvalue, &bucketno, &batchno);

    while (ammBatchContext && batchno == hashtable->curbatch && hashTuple == NULL) {
        if (!ExecHashEnsureTupleCapacity(hashtable, hashTupleSize, hashvalue, &bucketno, &batchno))
            break;
        hashTuple = (HashJoinTuple)dense_alloc_try(hashtable, hashTupleSize);
    }
    ...
    if (batchno == hashtable->curbatch) {
        ...
        if (!ammBatchContext)
            hashTuple = (HashJoinTuple)dense_alloc_try(hashtable, hashTupleSize);
        if (hashTuple == NULL)
            ExecHashTupleGrantError(hashtable, hashTupleSize, ERRCODE_PROGRAM_LIMIT_EXCEEDED);
        ...
        bool sysBusy = !ammBatchContext && gs_sysmemory_busy(hashtable->spaceUsed * dop, false);
        if (!ammBatchContext && (hashtable->spaceUsed + int64(hashtable->nbuckets_optimal * sizeof(HashJoinTuple)) >
         ...
         */
        Assert(batchno > hashtable->curbatch);
        ExecHashJoinSaveTuple(tuple, hashvalue, ExecHashGetInnerBatchFileSlot(hashtable, batchno));
...
void ExecHashTableReset(HashJoinTable hashtable)
{
    ...
    if (IsA(hashtable->batchCxt, AmmGranuleContext)) {
        hashtable->buckets = (HashJoinTuple*)ExecHashAmmTryAllocZero(
            hashtable->batchCxt, (Size)nbuckets * sizeof(HashJoinTuple));
        if (hashtable->buckets == NULL)
            ExecHashUnbatchableGrantError(hashtable, (Size)nbuckets * sizeof(HashJoinTuple));
    } else {
        hashtable->buckets = (HashJoinTuple*)palloc0(nbuckets * sizeof(HashJoinTuple));
    }
...
static void ExecHashBuildSkewHash(HashJoinTable hashtable, Hash* node, int mcvsToUse)
{
    ...
    if (get_attstatsslot(statsTuple,
         ...
         */
        if (IsA(hashtable->batchCxt, AmmGranuleContext)) {
            hashtable->skewBucket = (HashSkewBucket**)ExecHashAmmTryAllocZero(
                hashtable->batchCxt, (Size)nbuckets * sizeof(HashSkewBucket*));
            hashtable->skewBucketNums = (int*)ExecHashAmmTryAllocZero(
                hashtable->batchCxt, (Size)mcvsToUse * sizeof(int));
            if (hashtable->skewBucket == NULL || hashtable->skewBucketNums == NULL) {
        ...
        for (i = 0; i < mcvsToUse; i++) {
            ...
            if (IsA(hashtable->batchCxt, AmmGranuleContext)) {
                hashtable->skewBucket[bucket] =
                    (HashSkewBucket*)ExecHashAmmTryAlloc(hashtable->batchCxt, sizeof(HashSkewBucket));
            } else {
                hashtable->skewBucket[bucket] =
            ...
            if (hashtable->skewBucket[bucket] == NULL) {
                ...
                for (j = 0; j < nbuckets; j++)
                    pfree_ext(hashtable->skewBucket[j]);
                pfree_ext(hashtable->skewBucket);
                pfree_ext(hashtable->skewBucketNums);
                hashtable->spaceUsed -= nbuckets * sizeof(HashSkewBucket*) + mcvsToUse * sizeof(int) +
                    hashtable->nSkewBuckets * SKEW_BUCKET_OVERHEAD;
                hashtable->spaceUsedSkew -= nbuckets * sizeof(HashSkewBucket*) + mcvsToUse * sizeof(int) +
                    hashtable->nSkewBuckets * SKEW_BUCKET_OVERHEAD;
                hashtable->skewBucket = NULL;
                hashtable->skewBucketNums = NULL;
...
static bool ExecHashSkewTableInsert(HashJoinTable hashtable, TupleTableSlot* slot, uint32 hashvalue, int bucketNumber)
{
    ...
    int batchno;
    errno_t rc = EOK;
    bool storedInSkew = true;

    /* Preserve native skew allocation and removal behavior outside AMM. */
    if (!IsA(hashtable->batchCxt, AmmGranuleContext)) {
    ...
    /* Capacity checks can remove skew optimization before this tuple is linked. */
    bucketNumber = ExecHashGetSkewBucket(hashtable, hashvalue);
    if (bucketNumber != INVALID_SKEW_BUCKET_NO) {
        hashTuple->next = hashtable->skewBucket[bucketNumber]->tuples;
        hashtable->skewBucket[bucketNumber]->tuples = hashTuple;
    ...
    if (storedInSkew && ExecHashGetSkewBucket(hashtable, hashvalue) == INVALID_SKEW_BUCKET_NO)
        storedInSkew = false;
    return storedInSkew;
}
...
static void ExecHashIncreaseBuckets(HashJoinTable hashtable)
{
    ...
    if (IsA(hashtable->batchCxt, AmmGranuleContext)) {
        ...
        HashJoinTuple* newbuckets = (HashJoinTuple*)ExecHashAmmTryAllocZero(
            hashtable->batchCxt, (Size)oldnbuckets * 2 * sizeof(HashJoinTuple));
        ...
        for (int i = 0; i < oldnbuckets; i++) {
            HashJoinTuple htuple = oldbuckets[i];

            while (htuple != NULL) {
                HashJoinTuple next = htuple->next;
                int offset = (htuple->hashvalue >> hashtable->log2_nbuckets) & 1;

                ntotal++;
                if (offset == 1) {
                    htuple->next = newbuckets[i + oldnbuckets];
                    newbuckets[i + oldnbuckets] = htuple;
                    nmove++;
                } else {
                    htuple->next = newbuckets[i];
                    newbuckets[i] = htuple;
                }
                htuple = next;
            }
        ...
        pfree_ext(oldbuckets);
        hashtable->buckets = newbuckets;
        hashtable->nbuckets = oldnbuckets * 2;
...
static void* dense_alloc_try(HashJoinTable hashtable, Size size)
{
    ...
    if (size > HASH_CHUNK_THRESHOLD) {
        ...
        if (IsA(hashtable->batchCxt, AmmGranuleContext)) {
            Size chunkBytes = offsetof(HashMemoryChunkData, data) + size;

            newChunk = (HashMemoryChunk)ExecHashAmmTryAlloc(hashtable->batchCxt, chunkBytes);
        } else
        ...
        newChunk->used = 0;
        newChunk->ntuples = IsA(hashtable->batchCxt, AmmGranuleContext) ? HASH_CHUNK_DEDICATED_TUPLES : 0;
    ...
    if ((hashtable->chunks == NULL) || (hashtable->chunks->maxlen - hashtable->chunks->used) < size) {
        /* allocate new chunk and put it at the beginning of the list */
        if (IsA(hashtable->batchCxt, AmmGranuleContext)) {
            Size chunkPayload = ExecHashAmmChunkPayloadSize(hashtable, size);
            Size chunkBytes;

            if (chunkPayload == 0)
                return NULL;
            chunkBytes = offsetof(HashMemoryChunkData, data) + chunkPayload;
            if (!AmmGranuleContextCanAllocate(hashtable->batchCxt, chunkBytes))
                return NULL;
            newChunk = (HashMemoryChunk)AmmGranuleContextTryAlloc(hashtable->batchCxt, chunkBytes);
            if (newChunk != NULL)
                newChunk->maxlen = chunkPayload;
        } else
            newChunk = (HashMemoryChunk)MemoryContextAlloc(
