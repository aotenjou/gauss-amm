                ...
                if (batchno != hashtable->curbatch && node->hj_CurSkewBucketNo == INVALID_SKEW_BUCKET_NO) {
                    ...
                    if (ExecHashUsesAmmBatchFileSegments(hashtable))
                        ExecHashJoinSaveTuple(
                            tuple, hashvalue, ExecHashGetOuterBatchFileSlot(hashtable, batchno));
                    else
...
static TupleTableSlot* ExecHashJoinOuterGetTuple(PlanState* outerNode, HashJoinState* hjstate, uint32* hashvalue)
{
    ...
    if (curbatch == 0) {
        ...
        if (ExecHashUsesAmmBatchFileSegments(hashtable))
            file = *ExecHashGetOuterBatchFileSlot(hashtable, curbatch);
        else
            file = hashtable->outerBatchFile[curbatch];
...
static bool ExecHashJoinNewBatch(HashJoinState* hjstate)
{
    ...
    BufFile** innerSlot = NULL;
    BufFile** outerSlot = NULL;
    TupleTableSlot* slot = NULL;
    uint32 hashvalue;
    ...
    if (curbatch > 0) {
         ...
         */
        if (ExecHashUsesAmmBatchFileSegments(hashtable))
            outerSlot = ExecHashGetOuterBatchFileSlot(hashtable, curbatch);
        else
            outerSlot = &hashtable->outerBatchFile[curbatch];
        if (*outerSlot)
    ...
    while (curbatch < nbatch) {
        if (ExecHashUsesAmmBatchFileSegments(hashtable)) {
            innerSlot = ExecHashGetInnerBatchFileSlot(hashtable, curbatch);
            outerSlot = ExecHashGetOuterBatchFileSlot(hashtable, curbatch);
        } else {
            innerSlot = &hashtable->innerBatchFile[curbatch];
            outerSlot = &hashtable->outerBatchFile[curbatch];
        }
        if (*outerSlot != NULL && *innerSlot != NULL)
            break;
        if (*outerSlot && HJ_FILL_OUTER(hjstate))
            break; /* must process due to rule 1 */
        if (*innerSlot && HJ_FILL_INNER(hjstate))
            break; /* must process due to rule 1 */
        if (*innerSlot && nbatch != hashtable->nbatch_original)
            break; /* must process due to rule 2 */
        if (*outerSlot && nbatch != hashtable->nbatch_outstart)
    ...
    if (ExecHashUsesAmmBatchFileSegments(hashtable))
        ...
        outerSlot = &hashtable->outerBatchFile[curbatch];
    if (*outerSlot != NULL) {
        if (BufFileSeek(*outerSlot, 0, 0L, SEEK_SET))
            ereport(
