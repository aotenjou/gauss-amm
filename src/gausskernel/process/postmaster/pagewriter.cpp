...
static THR_LOCAL uint32 gs_amm_pending_writeback_batch_pages = 0;

static void gs_amm_retire_pending_writeback_batch(void)
{
...
static uint32 incre_ckpt_pgwr_flush_dirty_queue(WritebackContext *wb_context);
    ...
    bool *contain_hashbucket)
{
    ...
    for (uint32 buf_id = start; buf_id < end; buf_id++) {
        if (!GsAmmBufferIdIsActive((int)buf_id)) {
