...
int64 pgstat_get_session_spill_size(void)
{
    SessionLevelMemory *session_memory = t_thrd.shemem_ptr_cxt.mySessionMemoryEntry;

    if (session_memory == NULL)
        return 0;
    return gs_atomic_add_64(&session_memory->spillSize, 0);
}

int pgstat_get_session_spill_count(void)
{
    SessionLevelMemory *session_memory = t_thrd.shemem_ptr_cxt.mySessionMemoryEntry;

    if (session_memory == NULL)
        return 0;
    return gs_atomic_add_32(&session_memory->spillCount, 0);
}
