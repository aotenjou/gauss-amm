...
#define AMMGRANULE_NUM_FREELISTS 16

typedef struct AmmGranuleContext {
    AllocSetContext set; /* Keep AllocSet-compatible accounting prefix. */
    GsAmmGrantToken grant_token;
    Size maxBytes;
    Size allocatedBytes;
    Size liveBytes;
    void* chunks;
    void* freelist[AMMGRANULE_NUM_FREELISTS];
} AmmGranuleContext;

typedef AmmGranuleContext* AmmGranuleContextPtr;
...
#define MemoryContextIsValid(context)                                                                                 \
    ((context) != NULL &&                                                                                             \
        (IsA((context), AllocSetContext) || IsA((context), AsanSetContext) || IsA((context), StackAllocSetContext) || \
            ...
            IsA((context), MemalignSharedAllocSetContext) || IsA((context), AmmGranuleContext)))
