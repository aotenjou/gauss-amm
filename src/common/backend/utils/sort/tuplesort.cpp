...
/*
 * AMM grants are a hard pool limit.  Keep the small control allocations
 * required to start an external sort available while tuples consume the rest
 * of the grant.  The reserve covers a write buffer for every active tape plus
 * the LogicalTapeSet and BufFile control allocations.  The AMM context splits
 * the reserve when it is released at the transition to external sorting.
 */
#define AMM_SORT_SPILL_RESERVE_COUNT 8
#define AMM_SORT_SPILL_RESERVE_TAPESET_MIN_BYTES (4 * 1024)
#define AMM_SORT_SPILL_RESERVE_CONTROL_BYTES (2 * BLCKSZ)
...
struct Tuplesortstate {
    ...
    void* ammSpillReserves[AMM_SORT_SPILL_RESERVE_COUNT];
#ifdef PGXC
...
{
    ApplyGsAmmEffectiveSortGrant(state);
    ...
    if (!gs_sysmemory_busy(usedMem * state->dop, true) && state->maxMem > state->allowedMem &&
        ...
        if (spreadMem > state->allowedMem * MEM_AUTO_SPREAD_MIN_RATIO) {
            *growRatio = Min(*growRatio, 1 + (double)spreadMem / state->allowedMem);
            ...
            if (!IsA(state->sortcontext, AmmGranuleContext)) {
...
static void* AmmSortTryAlloc(MemoryContext context, Size size);
static void* AmmSortTryAllocZero(MemoryContext context, Size size);
static SortTuple* AmmSortTryResizeMemtuples(Tuplesortstate* state, int newmemtupsize);
static bool AmmSortEnsureTupleCapacity(Tuplesortstate* state, Size tuple_bytes);
static Size AmmSortIndexTupleBytes(TupleDesc tuple_desc, Datum* values, const bool* isnull);
...
static void AmmSortAllocationError(const char* allocation_name, Size bytes)
{
    ereport(ERROR,
        (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
            ...
            errmsg("sort cannot allocate %s within AMM memory grant: bytes %lu",
                allocation_name,
                (unsigned long)bytes)));
}
...
static void* AmmSortTryAlloc(MemoryContext context, Size size)
{
    if (!IsA(context, AmmGranuleContext))
...
static void* AmmSortTryAllocZero(MemoryContext context, Size size)
{
    void* pointer = AmmSortTryAlloc(context, size);
    errno_t rc;
...
static bool AmmSortReserveSpillMemory(Tuplesortstate* state)
{
    ...
    max_tapes = tuplesort_merge_order(state->allowedMem) + 1;
    reserve_bytes = (Size)max_tapes * BLCKSZ + AMM_SORT_SPILL_RESERVE_CONTROL_BYTES;
    reserve_bytes = Max(reserve_bytes, (Size)AMM_SORT_SPILL_RESERVE_TAPESET_MIN_BYTES);
    state->ammSpillReserves[0] = AmmSortTryAlloc(state->sortcontext, reserve_bytes);
    if (state->ammSpillReserves[0] == NULL) {
        AmmSortReleaseSpillMemory(state);
        return false;
    }
    USEMEM(state, GetMemoryChunkSpace(state->ammSpillReserves[0]));
...
static void AmmSortReleaseSpillMemory(Tuplesortstate* state)
{
    for (int i = 0; i < AMM_SORT_SPILL_RESERVE_COUNT; i++) {
        void* reserve = state->ammSpillReserves[i];

        if (reserve == NULL)
            continue;
        FREEMEM(state, GetMemoryChunkSpace(reserve));
        pfree_ext(reserve);
        state->ammSpillReserves[i] = NULL;
    }
}
...
static SortTuple* AmmSortTryResizeMemtuples(Tuplesortstate* state, int newmemtupsize)
{
    ...
    SortTuple* replacement;
    errno_t rc;

    if (!IsA(state->sortcontext, AmmGranuleContext))
        return NULL;
    replacement = (SortTuple*)AmmSortTryAlloc(state->sortcontext, new_bytes);
    if (replacement == NULL)
        return NULL;
    rc = memcpy_s(replacement, new_bytes, state->memtuples, state->memtupcount * sizeof(SortTuple));
    securec_check(rc, "\0", "\0");
    return replacement;
}
...
static bool AmmSortEnsureTupleCapacity(Tuplesortstate* state, Size tuple_bytes)
{
    ...
    if (state->tuplecontext == NULL || !IsA(state->tuplecontext, AmmGranuleContext))
        return true;
    if (AmmGranuleContextCanAllocate(state->tuplecontext, tuple_bytes))
        return true;

    oldcontext = MemoryContextSwitchTo(state->sortcontext);
    if (state->status == TSS_INITIAL)
        inittapes(state, true);
    if (state->status == TSS_BUILDRUNS && state->memtupcount > 0) {
...
static Size AmmSortIndexTupleBytes(TupleDesc tuple_desc, Datum* values, const bool* isnull)
{
    ...
    for (attribute_no = 0; attribute_no < tuple_desc->natts; attribute_no++) {
        if (isnull[attribute_no]) {
            infomask |= INDEX_NULL_MASK;
            break;
        }
    }
    return MAXALIGN(IndexInfoFindDataOffset(infomask) +
        heap_compute_data_size(tuple_desc, values, isnull));
}
...
static Tuplesortstate* tuplesort_begin_common(int64 workMem, bool randomAccess, SortCoordinate coordinate)
{
    ...
    state = (Tuplesortstate*)AmmSortTryAllocZero(sortcontext, sizeof(Tuplesortstate));
    if (state == NULL)
        AmmSortAllocationError("sort state", sizeof(Tuplesortstate));
    ...
    state->memtuples = (SortTuple*)AmmSortTryAlloc(
        sortcontext, (Size)state->memtupsize * sizeof(SortTuple));
    while (state->memtuples == NULL && state->memtupsize > MINORDER * 2) {
        state->memtupsize = Max(MINORDER * 2, state->memtupsize / 2);
        state->memtuples = (SortTuple*)AmmSortTryAlloc(
            sortcontext, (Size)state->memtupsize * sizeof(SortTuple));
    }
    if (state->memtuples == NULL)
        AmmSortAllocationError("initial tuple array", (Size)state->memtupsize * sizeof(SortTuple));
    ...
    if (!AmmSortReserveSpillMemory(state))
        AmmSortAllocationError("external sort control memory", AMM_SORT_SPILL_RESERVE_TAPESET_MIN_BYTES);
    ...
    int dop)
{
    ...
    state->sortKeys = (SortSupport)AmmSortTryAllocZero(
        state->sortcontext, (Size)nkeys * sizeof(SortSupportData));
    if (state->sortKeys == NULL)
        AmmSortAllocationError("sort keys", (Size)nkeys * sizeof(SortSupportData));
    ...
    Oid datumType, Oid sortOperator, Oid sortCollation, bool nullsFirstFlag, int workMem, bool randomAccess)
{
    ...
    state->onlyKey = (SortSupport)AmmSortTryAllocZero(state->sortcontext, sizeof(SortSupportData));
    if (state->onlyKey == NULL)
        AmmSortAllocationError("sort key", sizeof(SortSupportData));
...
static bool grow_memtuples(Tuplesortstate* state)
{
    ...
    newmemtupsize = (int)(memtupsize * growRatio);
    if (IsA(state->sortcontext, AmmGranuleContext)) {
        SortTuple* replacement = AmmSortTryResizeMemtuples(state, newmemtupsize);

        if (replacement == NULL)
            goto noalloc;
        FREEMEM(state, GetMemoryChunkSpace(state->memtuples));
        pfree_ext(state->memtuples);
        state->memtuples = replacement;
...
void tuplesort_puttupleslotontape(Tuplesortstate* state, TupleTableSlot* slot)
{
    ...
    if (!AmmSortEnsureTupleCapacity(state, ExecFetchSlotMinimalTuple(slot)->t_len))
        AmmSortAllocationError("input tuple", ExecFetchSlotMinimalTuple(slot)->t_len);
...
void tuplesort_puttupleslot(Tuplesortstate* state, TupleTableSlot* slot)
{
     ...
     */
    if (!AmmSortEnsureTupleCapacity(state, tuple->t_len))
        AmmSortAllocationError("input tuple", tuple->t_len);
...
void TuplesortPutheaptuple(Tuplesortstate* state, HeapTuple tup)
{
    ...
    if (!AmmSortEnsureTupleCapacity(state, ((HeapTuple)tup)->t_len + HEAPTUPLESIZE))
        AmmSortAllocationError("input tuple", ((HeapTuple)tup)->t_len + HEAPTUPLESIZE);
...
{
    Size tuple_bytes = AmmSortIndexTupleBytes(RelationGetDescr(rel), values, isnull);
    MemoryContext oldcontext;
    SortTuple stup;

    if (!AmmSortEnsureTupleCapacity(state, tuple_bytes))
        AmmSortAllocationError("index tuple", tuple_bytes);
...
void tuplesort_putdatum(Tuplesortstate* state, Datum val, bool isNull)
{
    ...
    if (isNull || !state->tuples) {
        ...
        Size tuple_bytes = datumGetSize(val, false, state->datumTypeLen);

        if (!AmmSortEnsureTupleCapacity(state, tuple_bytes))
            AmmSortAllocationError("input datum", tuple_bytes);
...
static void inittapestate(Tuplesortstate *state, int maxTapes)
{
    ...
    state->mergeactive = (bool *)AmmSortTryAllocZero(state->sortcontext, maxTapes * sizeof(bool));
    state->tp_fib = (int *)AmmSortTryAllocZero(state->sortcontext, maxTapes * sizeof(int));
    state->tp_runs = (int *)AmmSortTryAllocZero(state->sortcontext, maxTapes * sizeof(int));
    state->tp_dummy = (int *)AmmSortTryAllocZero(state->sortcontext, maxTapes * sizeof(int));
    state->tp_tapenum = (int *)AmmSortTryAllocZero(state->sortcontext, maxTapes * sizeof(int));
    if (state->mergeactive == NULL || state->tp_fib == NULL || state->tp_runs == NULL ||
        state->tp_dummy == NULL || state->tp_tapenum == NULL)
        AmmSortAllocationError("tape metadata", (Size)maxTapes *
            (sizeof(bool) + sizeof(int) * 4));
...
static void init_slab_allocator(Tuplesortstate *state, int numSlots)
{
    if (numSlots > 0) {
        ...
        state->slabMemoryBegin = (char *)AmmSortTryAlloc(
            state->sortcontext, (Size)numSlots * SLAB_SLOT_SIZE);
        if (state->slabMemoryBegin == NULL)
            AmmSortAllocationError("merge slab", (Size)numSlots * SLAB_SLOT_SIZE);
...
static void mergeruns(Tuplesortstate* state)
{
    ...
    state->memtuples = (SortTuple*)AmmSortTryAlloc(
        state->sortcontext, (Size)numInputTapes * sizeof(SortTuple));
    if (state->memtuples == NULL)
        AmmSortAllocationError("merge tuple array", (Size)numInputTapes * sizeof(SortTuple));
    USEMEM(state, GetMemoryChunkSpace(state->memtuples));
...
static void* readtup_alloc(Tuplesortstate *state, Size tuplen)
{
    ...
    if (tuplen > SLAB_SLOT_SIZE || !state->slabFreeHead) {
        void* pointer = AmmSortTryAlloc(state->sortcontext, tuplen);

        if (pointer == NULL)
            AmmSortAllocationError("merge tuple", tuplen);
        return pointer;
    } else {
...
static void copytup_index(Tuplesortstate* state, SortTuple* stup, void* tup)
{
    ...
    /* copy the tuple into sort storage */
    newtuple = (IndexTuple)AmmSortTryAlloc(state->tuplecontext, tuplen);
    if (newtuple == NULL)
        AmmSortAllocationError("index tuple", tuplen);
...
static void readtup_stream(Tuplesortstate* state, SortTuple* stup, int tapenum, unsigned int len)
{
    ...
    /* copy the tuple into sort storage */
    source = ExecFetchSlotMinimalTuple(slot);
    if (IsA(state->sortcontext, AmmGranuleContext) &&
        !AmmGranuleContextCanAllocate(state->sortcontext, source->t_len))
        AmmSortAllocationError("stream merge tuple", source->t_len);
    tuple = ExecCopySlotMinimalTuple(slot);
    stup->tuple = (void*)tuple;
    USEMEM(state, GetMemoryChunkSpace(tuple));
    /* set up first-column key value */
            ...
            case RESPONSE_EOF:
                ...
                if (ans == RESPONSE_EOF) {
                    if (pgxc_node_receive(1, &conn, &timeout, true)) {
                        ...
                        bool *has_checked = (bool*)AmmSortTryAllocZero(
                            state->sortcontext, sizeof(bool) * combiner->conn_count);
                        int has_err_idx = -1;
                        int cnt = 0;
                        const int loopnum = 10;

                        if (has_checked == NULL)
                            AmmSortAllocationError("remote merge state", sizeof(bool) * combiner->conn_count);
...
static void readtup_datanode(Tuplesortstate* state, SortTuple* stup, int tapenum, unsigned int len)
{
    ...
    /* copy the tuple into sort storage */
    MinimalTuple source = ExecFetchSlotMinimalTuple(slot);
    if (IsA(state->sortcontext, AmmGranuleContext) &&
        !AmmGranuleContextCanAllocate(state->sortcontext, source->t_len))
        AmmSortAllocationError("remote merge tuple", source->t_len);
    tuple = ExecCopySlotMinimalTuple(slot);
    stup->tuple = (void*)tuple;
    USEMEM(state, GetMemoryChunkSpace(tuple));
    /* set up first-column key value */
    ...
    Oid* sortCollations, const bool* nullsFirstFlags, void* combiner, int workMem)
{
    ...
    state->sortKeys = (SortSupport)AmmSortTryAllocZero(
        state->sortcontext, (Size)nkeys * sizeof(SortSupportData));
    if (state->sortKeys == NULL)
        AmmSortAllocationError("merge sort keys", (Size)nkeys * sizeof(SortSupportData));
    ...
    state->mergeactive = (bool*)AmmSortTryAllocZero(state->sortcontext, conn_count * sizeof(bool));
    state->tp_runs = (int*)AmmSortTryAllocZero(state->sortcontext, conn_count * sizeof(int));
    state->tp_dummy = (int*)AmmSortTryAllocZero(state->sortcontext, conn_count * sizeof(int));
    state->tp_tapenum = (int*)AmmSortTryAllocZero(state->sortcontext, conn_count * sizeof(int));
    if (state->mergeactive == NULL || state->tp_runs == NULL || state->tp_dummy == NULL ||
        state->tp_tapenum == NULL)
        AmmSortAllocationError("parallel merge metadata", (Size)conn_count *
            (sizeof(bool) + sizeof(int) * 3));
