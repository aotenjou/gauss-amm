...
DEFAULT_GSHOME = "/opt/ammgs/native"
...
def start_gsbench(args, workers, duration, out_file):
    cmd = (
        "runuser -u ammdb -- sh -c 'cd %s && GSBENCH_PASSWORD=%s %s run 101 -c %s/native.cfg "
...
def main():
    ...
    p.add_argument("--kernel", choices=["amm", "native"], default="native")
    ...
    if st.get("ap_multipass_only"):
        print("ERROR: ap_multipass_only=true; restart the server for a clean run.",
              file=sys.stderr)
        return 2
    ...
    for res in results:
        tps = tps_from_samples(res.get("samples", []))
        res["tps_windows"] = tps
        res["tps_jitter_pct"] = jitter_pct(tps)
